...
func (t *Translater) Visit(n *html.Node) string {
	if !shouldVisit(n) {
		return ""
	}
	switch n.Type {
...
func (t *Translater) visitChildren(n *html.Node) []string {
	content := []string{}
...
func main() {
	bytes, _ := os.ReadFile("test.html")
	doc, _ := html.Parse(strings.NewReader(string(bytes)))
	t := &Translater{}
	text := t.Visit(doc)
	text = cleanup(text)
	fmt.Printf("\napprox num tokens: %d\n", len(text)/4)
	if err := os.WriteFile("test.md", []byte(text), 0644); err != nil {
		panic(err)
	}
}
